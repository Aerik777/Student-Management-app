    ...
    <div className="p-8">
      ...
      <div className="grid gap-4">
        {myBooks.map((record: any) => {
          ...
          return (
            <div key={record._id} className="bg-white p-4 rounded-lg shadow border flex justify-between items-center">
              <div>
                <h3 className="font-bold text-lg">{record.bookId.title}</h3>
                <p className="text-gray-500 text-sm">Issued on: {new Date(record.issueDate).toLocaleDateString()}</p>
              </div>
              <div className="text-right">
                <p className={`text-sm font-semibold ${isOverdue ? 'text-red-600' : 'text-green-600'}`}>
                  Due: {new Date(record.dueDate).toLocaleDateString()}
                </p>
                {isOverdue && <span className="text-[10px] bg-red-100 text-red-600 px-2 py-1 rounded">OVERDUE</span>}
              </div>
            </div>
          );
        })}
        {myBooks.length === 0 && <p className="text-gray-400">You haven't borrowed any books yet.</p>}
      </div>
    </div>
  );
}